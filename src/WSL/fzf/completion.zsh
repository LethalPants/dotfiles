...
if ! declare -f _fzf_compgen_path > /dev/null; then
  _fzf_compgen_path() {
    echo "$1"
    command rg --files --hidden --follow "$1"
    #command find -L "$1" \
...
if ! declare -f _fzf_compgen_dir > /dev/null; then
  _fzf_compgen_dir() {
    command find -L "$1" \
      -name .git -prune -o -name .hg -prune -o -name .svn -prune -o -name .npm -prune -o \
      -name node_modules -prune -o -name .vscode-server -prune -o -name .vscode -prune -o \
      -name .pyenv -prune -o -name .oh-my-zsh -prune -o -name .local -prune -o \
      -name .vscode-remote-containers -prune -o -name .landscape -prune -o \
      -name .Bloomberg -prune -o -name .cache -prune -o \
      -name .vim -prune -o -name .dbus -prune -o \
      -name .kube -prune -o -name .tsh -prune -o -name .github -prune -o \
      -name .rde -prune -o \
      -type d \
      -a -not -path "$1" -print 2> /dev/null | sort -r | sed 's@^\./@@'
  }
fi
...
if ! declare -f __fzf_list_hosts > /dev/null; then
  __fzf_list_hosts() {
    setopt localoptions nonomatch
    command batcat <(command grep -oE '^[[a-z0-9.,:-]+' ~/.ssh/known_hosts 2> /dev/null | tr ',' '\n' | tr -d '[' | awk '{ print $1 " " $1 }') |
...
fzf-completion() {
  ...
  if [ ${#tokens} -gt 1 -a "$tail" = "$trigger" ]; then
    d_cmds=(${=FZF_COMPLETION_DIR_COMMANDS:-cd pushd rmdir code})
