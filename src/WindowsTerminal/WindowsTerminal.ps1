...
function Set-Startship-Config {
  $MainConfigPath = Join-Path -Path $DotfilesWorkFolder -ChildPath "WSL" | Join-Path -ChildPath "startship.toml";
  $StarshipPath = Join-Path -Path $HOME -ChildPath "startship.toml";


  Write-Host "Copying final Starship configuration file..." -ForegroundColor "Green";
  Copy-Item $MainConfigPath -Destination $StarshipPath;

  Write-Host "Starship was successfully configured." -ForegroundColor "Green";
}
...
choco install -y "starship" --params "/NoAutoCrlf /WindowsTerminal /NoShellIntegration /SChannel";
Set-PowerShell-Profile;
Set-Startship-Config;
